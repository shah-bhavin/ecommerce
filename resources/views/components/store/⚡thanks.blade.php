...
<main class="pt-32 pb-24 px-6 md:px-12 lg:px-24">
  @if(!$order)
      <!-- ERROR STATE -->
      <section class="max-w-6xl mx-auto text-center mb-20">
          <div class="mb-8 inline-block">
              <div class="w-20 h-20 rounded-xl flex items-center justify-center bg-red-100 text-red-600 mb-4 mx-auto">
                  <x-lucide-alert-circle class="size-10" />
              </div>
          </div>
          <h1 class="hero-medium">Order Not Found</h1>
          <p class="font-body text-on-surface-variant max-w-2xl mx-auto text-lg leading-relaxed mb-10">
              We couldn't find the order details you're looking for. It may have been moved or the link might be incorrect.
          </p>
        ...
        <div class="bg-surface-container-low p-8 md:p-12">
          ...
          <div class="space-y-4 pt-8 border-t border-outline-variant/30">
            <div class="flex justify-between items-center text-xs uppercase tracking-widest">
              ...
              <span>₹{{ number_format(@$this->order->subtotal) }}</span>
            </div>

            <div class="flex justify-between items-center text-xs uppercase tracking-widest">
              <span class="text-on-surface-variant">Shipping</span>
              @if($this->order->shipping_charges == 0)
              <span class="text-green-600 font-bold uppercase tracking-widest text-[10px]">Complimentary</span>
              @else
              <span class="text-secondary">₹{{ number_format(@$this->order->shipping_charges) }}</span>
              @endif
            ...
            <div class="flex justify-between items-center text-xs uppercase tracking-widest">
              ...
              @else
              <span class="text-secondary"> - ₹{{ number_format(@$this->order->discount_amount) }}</span>
              @endif
            ...
            <div class="flex justify-between items-center pt-4 border-t border-on-surface/10">
              ...
              <span class="font-headline text-2xl text-primary">₹{{ number_format(@$this->order->total, 2) }}</span>
            </div>
          </div>
        </div>
